...
<!-- or if you need to preserve state of child components -->
<div class:hidden={!isActive} aria-hidden={!isActive}>
  <!-- ... -->
</div>
...
<script lang="ts">
  ...
  const { isActive, close }: ModalProps<Result> = $props()
</script>
...
<script>
  const { isActive, close, content } = $props()
</script>
